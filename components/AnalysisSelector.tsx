...
export function AnalysisSelector({ onSelect, onRunAnalysis, isAnalyzing }: AnalysisSelectorProps) {
    const options = [
        ...
        { id: 'cfa-select', title: 'CFA (Sắp ra mắt)', desc: 'Phân tích nhân tố khẳng định (Bảo trì)', icon: Network, action: 'select', colors: 'border-gray-200 bg-gray-50 opacity-60 cursor-not-allowed' },
        { id: 'sem-select', title: 'SEM (Sắp ra mắt)', desc: 'Mô hình cấu trúc tuyến tính (Bảo trì)', icon: Layers, action: 'select', colors: 'border-gray-200 bg-gray-50 opacity-60 cursor-not-allowed' }
    ];
